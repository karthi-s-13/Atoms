...
export function useRealtimeSimulation() {
  ...
  const controlsRef = useRef(DEFAULT_CONFIG);
  ...
  useEffect(() => {
    let isMounted = true;
    const socketUrl = getBackendWsUrl("/ws");
    ...
    const publish = (incomingSnapshot) => {
      ...
      if (now - lastDashboardPublishRef.current >= 180) {
        ...
        startTransition(() => {
          controlsRef.current = snapshot.config;
    ...
    const scheduleReconnect = () => {
      ...
      const delay = Math.min(
        ...
        RECONNECT_BASE_DELAY_MS * (2 ** Math.max(reconnectAttemptRef.current - 1, 0)),
      );
  ...
  const seedLocalState = (nextConfig = DEFAULT_CONFIG) => {
    ...
    controlsRef.current = seededSnapshot.config;
    setControls(seededSnapshot.config);
    setModeBenchmarks(DEFAULT_MODE_BENCHMARKS);
  };

  const updateConfig = (partialConfig) => {
    const nextConfig = mergeConfig(controlsRef.current, partialConfig);
    controlsRef.current = nextConfig;
