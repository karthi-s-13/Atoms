...
BASE_DETECTOR_MODEL_SOURCE = str(BASE_DETECTOR_MODEL_PATH) if BASE_DETECTOR_MODEL_PATH.exists() else "yolov8n.pt"
...
class VehicleDetector:
    ...
    async def _get_model(self) -> YOLO:
        ...
        async with self._lock:
            if self._model is None:
                self._model = await asyncio.to_thread(YOLO, BASE_DETECTOR_MODEL_SOURCE)
        return self._model
    ...
    async def _get_tracking_model(self, stream_key: str) -> YOLO:
        ...
        async with self._lock:
            if stream_key not in self._tracking_models:
                ...
                self._tracking_models[stream_key] = await asyncio.to_thread(YOLO, BASE_DETECTOR_MODEL_SOURCE)
            self._tracking_last_used[stream_key] = time.time()
    ...
    def _get_tracking_model_sync(self, stream_key: str) -> YOLO:
        ...
        if model is None:
            self._cleanup_tracking_models()
            model = YOLO(BASE_DETECTOR_MODEL_SOURCE)
