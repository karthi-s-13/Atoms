...
DEMO_START_POINTS = [
    {"id": "S1", "name": "North Wharf Standby", "lat": 13.0898, "lng": 80.2790, "nearby_junctions": ["J1", "J2"]},
    {"id": "S2", "name": "Civic Rescue Bay", "lat": 13.0860, "lng": 80.2622, "nearby_junctions": ["J1", "J2"]},
    {"id": "S3", "name": "Metro Command Point", "lat": 13.0798, "lng": 80.2540, "nearby_junctions": ["J1", "J2"]},
    {"id": "S4", "name": "Riverfront Dispatch", "lat": 13.0735, "lng": 80.2678, "nearby_junctions": ["J1", "J2"]},
]
DEMO_TRIGGER_DISTANCE_METERS = 10.0
DEMO_APPROACH_JUNCTION_ID = "J1"
DEMO_PRE_DETECTION_JUNCTIONS = ("J1", "J2")
DEMO_EMERGENCY_ROUTE_NODES = ("J1", "J3", "J4", "J5")
...
class TrafficPlatformService:
    ...
    def _smooth_metric(self, junction_id: str, metric: str, current: float, junction_map: dict[str, dict[str, Any]]) -> float:
        previous = _coerce_float(self.state[junction_id]["metrics"].get(metric), default=current)
        alpha = METRIC_ALPHA.get(metric, 0.3)
        temporal = (1.0 - alpha) * previous + alpha * current
        neighbor_avg = self._neighbor_average(junction_id, metric, junction_map)
        if neighbor_avg > 0.0:
            temporal = temporal + (0.2 * neighbor_avg)
        if metric == "density":
            return round(_clip(temporal, 0.0, 1.0), 3)
        return round(max(temporal, 0.0), 2)
    ...
    def _prediction(self, junction_id: str, metrics: dict[str, float], confidence: dict[str, float]) -> dict[str, Any]:
        ...
        trend = self._trend_prediction(vehicle_history)
        moving_avg = self._moving_average(vehicle_history)
        recent_delta = self._recent_delta(vehicle_history)
        predicted = (0.45 * trend) + (0.35 * moving_avg) + (0.20 * (vehicle_history[-1] + recent_delta))
        previous_prediction = _coerce_float(self.state[junction_id]["prediction"].get("predicted_load"), default=predicted)
        smoothed_prediction = round((0.7 * previous_prediction) + (0.3 * predicted), 2)
        clipped = round(_clip(smoothed_prediction, 0.0, max(max(vehicle_history) * 1.45, 20.0)), 2)
    ...
    def get_demo_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "activation_junction_id": DEMO_APPROACH_JUNCTION_ID,
            "pre_detection_junctions": list(DEMO_PRE_DETECTION_JUNCTIONS),
            "emergency_route_nodes": list(DEMO_EMERGENCY_ROUTE_NODES),
        }
    ...
    def start_emergency_demo(self, start_point_id: str) -> dict[str, Any]:
        ...
        activation_junction = self._demo_junction(DEMO_APPROACH_JUNCTION_ID)
        hospital = self._find_nearest_hospital(activation_junction["lat"], activation_junction["lng"])
        approach_route_coords = [
            self._coord_record(
        ...
        self._append_emergency_alert(
            f"Ambulance dispatched from {start_point['id']} and moving toward {activation_junction['id']} for live detection.",
            stage="approach",
        )
    ...
    def _fallback_metrics(self, junction_id: str, junction_map: dict[str, dict[str, Any]]) -> dict[str, float]:
        ...
        base = junction["base_metrics"]
        fallback["vehicle_count"] = float(round(sum(neighbor_values["vehicle_count"]) / len(neighbor_values["vehicle_count"]))) if neighbor_values["vehicle_count"] else float(base["vehicle_count"])
        fallback["queue_length"] = float(round(sum(neighbor_values["queue_length"]) / len(neighbor_values["queue_length"]))) if neighbor_values["queue_length"] else float(base["queue"])
        fallback["density"] = float(sum(neighbor_values["density"]) / len(neighbor_values["density"])) if neighbor_values["density"] else float(base["density"])
        fallback["flow_count"] = float(round(sum(neighbor_values["flow_count"]) / len(neighbor_values["flow_count"]))) if neighbor_values["flow_count"] else float(base["flow_count"])
        fallback["average_speed_kmph"] = float(sum(neighbor_values["average_speed_kmph"]) / len(neighbor_values["average_speed_kmph"])) if neighbor_values["average_speed_kmph"] else float(base["average_speed_kmph"])
        return fallback
